...
/**
 * 基类作用: 1.对常规方法进行封装 2.对常规属性，常规方法进行封装
 * 
 * BaseFragment抽取前分析（核心）:
 * 
 * 一、显示页面分析 View Fragment共性 -->页面共性 -->视图的展示 任何应用其实就只有4中页面类型 1.加载页面 2.错误页面 3.空页面
 * 4.成功页面 1.2.3.三种页面的一个应用基本是固定的 每一个fragment对应的页面4.就不一样 进入应用的时候显示1.
 * 2.3.4.需要加载数据之后才知道显示哪个
 * 
 * 二、数据加载的流程 Model 1.触发加载，进入页面开始加载/点击某个按钮的时候加载 2.异步加载数据 -->显示加载视图 3.处理加载结果 1.成功
 * -->显示成功视图 2.失败 1.数据为空-->显示空视图 2.数据加载失败-->显示加载失败的视图
 * 
...
public abstract class BaseFragment extends Fragment
{
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		{
			/*
			 * 匿名内部类继承了LoadingPager类， 父类的引用指向子类的对象 重写LoadingPager的类方法：
			 */
